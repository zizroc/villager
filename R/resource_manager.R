...
resource_manager <- R6::R6Class("resource_manager",
                                ...
                                public = list(
                                              ...
                                              add_resource = function(new_resource) {
                                                private$resources <- append(private$resources, new_resource)
                                              },
                                              ...
                                              remove_resource = function(name) {
                                                ...
                                                private$resources<-private$resources[- resource_index]
                                              },
                                              ...
                                              get_resource_index = function(name) {
                                                for (i in 1:length(private$resources)) {
                                                  if (private$resources[[i]]$name == name) {
                                                    return (i)
                                                  }
                                                }
                                              },
                                              ...
                                              get_states = function() {
                                                ...
                                                if(resource_count > 0) {
                                                  ...
                                                  test_resource <- private$resources[[1]]$as_table()
                                                  colnames(state_table) <- names(test_resource)
                                                  for (i in 1:resource_count) {
                                                    state_table[i, ] <-  private$resources[[i]]$as_table()
                                                  }
                                                }
                                                return(state_table)
                                              },
